...
public class ReservaServiceImpl implements ReservaService {
	...
	private Veiculo getVeiculoDisponivel(Long modelo, LocalDate dataInicial, LocalDate dataFinal) {
		return veiculoReposity.findVeiculoDisponivel(modelo, dataInicial, dataFinal)
				.orElseThrow(() -> new VeiculoIndisponivelRuntimeException(
						String.format("nenhum veículo para o modelo = %s disponível", modelo)));
	}
	...
	public Reserva findbyId(Long id) {
		return reservaRepository.findById(id)
				.orElseThrow(() -> new NotFoundRuntimeException("Nenhuma reserva para id = " + id));
	}
